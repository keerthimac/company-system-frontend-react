...
const SupervisorMainDashboardPage = () => {
  const widgets = [
    ...
    {
      title: "Tools Assigned Today",
      value: "17",
      color: "bg-success text-success-content",
      icon: <FaToolsIcon className="w-8 h-8" />,
      link: "tools",
    },
  ];
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-base-content/70">
          Overview of your current tasks and machine/tool status.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {widgets.map((widget, index) => (
          <Link
            key={index}
            to={widget.link || "#"}
            className={`card shadow-lg hover:shadow-xl transition-shadow duration-300 ${widget.color}`}
          >
            <div className="card-body flex-row items-center p-6">
              {" "}
              {/* Use flex-row for icon and text side by side */}
              <div className="mr-4 p-3 bg-black bg-opacity-20 rounded-full">
                {widget.icon}
              </div>
              <div>
                <h3 className="card-title text-3xl font-bold">
                  {widget.value}
                </h3>
        ...
        <div className="card-body">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {quickLinks.map((link, index) => (
              ...
              >
                <span className="text-xl mb-1 sm:mb-0 sm:mr-2">
                  {link.icon}
                </span>
                {link.label}
              </Link>
      ...
      <div className="card bg-base-100 shadow-lg">
        <div className="card-body">
          ...
          <p className="text-base-content/70">
            Further content specific to the Supervisor's main dashboard (e.g.,
            charts, recent assignments) can go here. This area could display a
            list of recently assigned tools, machines needing maintenance based
            on supervisor reports, etc.
          </p>
          {/* Example of a placeholder for a chart */}
          <div className="mt-4 p-4 bg-base-200 rounded-box text-center">
            <p className="text-lg font-medium">
              Machine Status Overview Chart (Placeholder)
            </p>
